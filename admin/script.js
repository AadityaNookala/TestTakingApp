...
const testCategories = (await sendAPI("GET", `${baseUrl}/categories`)).data
  .data;
...
categoriesContainer.addEventListener("click", function (e) {
  ...
  if (editTestCategory) {
    ...
    editCategoryName.addEventListener("click", async function () {
      ...
      editCategoryName.remove();
      cancel.remove();
      buttonsWrapper.closest(".test-categories-showing").dataset.categoryName =
        newTestCategoryName;
      const userData = (await sendAPI("GET", `${baseUrl}/user`)).data.data;
      userData.forEach((el) => {
      ...
      await sendAPI(
        "PATCH",
        `${baseUrl}/updateTestCategory/${oldTestCategoryName}`,
...
addNewTestsCategory.addEventListener("click", function () {
...
const showUsers = async function () {
  userData.forEach((element) => {
    ...
    const user = usersContainer.lastElementChild;
...
const showTestCategoriesAndUsersInScores = function () {
...
showUsers();
showTestCategoriesAndUsersInScores();
showCategories();
...
categoriesContainerUsers.forEach((element) => {
  Sortable.create(element, {
    ...
    async onAdd(e) {
      ...
      const object = userData.find((element) => element.userName === userName);
