    ...
    score.enteredSentence.forEach((el, i) => {
      ...
      el.indexOfActualSentence.forEach((element, k) => {
        if (
          ...
          actualSentence = version.sentences[el.version - 1];
        } else {
          actualSentence = test.sentences[element];
        }
        let mistakenSentence = actualSentence.slice();
        const split = mistakenSentence.split(" ");
        el.mistakenAnswers[k].forEach((_, j) => {
          const punctuationRegex = /[!"#$%&'()*+,-./:;<=>?@[\]^_`{|}~“”‘’]/g;
          const punctuations = actualSentence
            .split(" ")
            [test.answers[k][j]].match(punctuationRegex);
          split[test.answers[element][j]] = `${el.mistakenAnswers[k][j]}${
...
// (async () => {
//   const allScores = await scoresData.find();
//   const newScores = allScores.map((el) => {
//     el.enteredSentence = el.enteredSentence.map((element) => {
//       element = element.toObject();
//       const temp = element.mistakenWords;
//       delete element.mistakenWords;
//       element.mistakenAnswers = temp;
//       return element;
//     });
//     return el;
//   });
//   await scoresData.deleteMany({});
//   await scoresData.insertMany(newScores);
// })();
